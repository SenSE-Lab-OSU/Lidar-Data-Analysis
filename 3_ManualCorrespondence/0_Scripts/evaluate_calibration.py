...
def load_transform(path: str):
    """
    Return (T, c, delta) from a JSON file.

    T is the 4×4 T_cam_lidar matrix.
    c and delta are optional scale corrections written by manual_calibrate.py
    when solved with --solve-c / --solve-delta; they default to 1.0 if the
    file doesn't contain those keys (older calibration_result.json files, or
    hand-built transform_*.json files, both still work unchanged).

    Accepted formats
    ----------------
    • {"translation": [tx,ty,tz], "quaternion": [qx,qy,qz,qw], "c": ..., "delta": ...}
    • {"matrix": [[row0], [row1], [row2], [row3]], "c": ..., "delta": ...}   (4×4 row-major)
    """
    with open(path) as f:
        data = json.load(f)
...
def reproject(pts3d: np.ndarray, T: np.ndarray,
    ...
    Both default to 1.0 — same behaviour as before these were added.
    ...
    proj, _ = cv2.projectPoints(
        pts.astype(np.float64), rvec, tvec, K_eff, dist)
...
def evaluate_transform(transform_label: str, T: np.ndarray,
                       ...
                       dist: np.ndarray,
                       c: float = 1.0, delta: float = 1.0) -> list[dict]:
    """
    ...
    for pair in pairs:
        ...
        pts3d = np.array([cc['pt3d'] for cc in corr], dtype=np.float64)
        pts2d = np.array([cc['pt2d'] for cc in corr], dtype=np.float64)

        proj  = reproject(pts3d, T, K, dist, c=c, delta=delta)
        errs  = np.linalg.norm(proj - pts2d, axis=1)

        for i, (cc, (pu, pv), err) in enumerate(zip(corr, proj, errs)):
            rows.append({
...
def main():
    ...
    for path, label in zip(args.transforms, labels):
        ...
        rows = evaluate_transform(label, T, pairs, K, dist, c=c, delta=delta)
