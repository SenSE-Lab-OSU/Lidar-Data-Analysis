...
def load_transform(path: str):
    """
    Load a 4×4 T_cam_lidar matrix, plus optional scale corrections, from:
      • a .npy file containing a (4,4) array (c=delta=1.0, npy has no room
        for extra scalars), OR
      • a .json file with keys "translation" and "quaternion", optionally
        also "c" and "delta" (written by manual_calibrate.py when solved
        with --solve-c / --solve-delta).
    ...
    if ext == '.npy':
        ...
        return T.astype(np.float64), 1.0, 1.0
...
def project_points(pts3d: np.ndarray, T: np.ndarray,
    ...
    c     : optional focal-length scale factor (fx,fy *= c; cx,cy untouched).
    delta : optional LiDAR radial-scale factor (pts3d *= delta before R,t).
    Both default to 1.0 — identical to the original behaviour.

    Returns:
      px, py  : (N,) pixel coordinates (float)
      depth   : (N,) depth in camera Z axis (metres), computed on the
                delta-corrected points so it stays consistent with px,py
    ...
    R   = T[:3, :3]
    t   = T[:3,  3]
    cam = (R @ pts_eff.T).T + t          # (N,3) in camera frame
    ...
    proj, _ = cv2.projectPoints(
        pts_eff[in_front].astype(np.float64),
...
def lidar_on_image(img_bgr: np.ndarray, pts3d: np.ndarray,
                   ...
                   c: float = 1.0, delta: float = 1.0) -> np.ndarray:
    """
    ...
    c, delta : optional scale corrections, see project_points().
    """
    canvas = img_bgr.copy()
    h, w   = canvas.shape[:2]
...
def image_on_lidar(img_bgr: np.ndarray, pts3d: np.ndarray,
    ...
    c, delta : optional scale corrections, see project_points(). Note the
    spatial layout of this front view still uses the raw pts3d positions
    (delta only affects which camera pixel each point samples color from,
    not where it's drawn in this view) — that's a cosmetic choice, not a
    correctness issue for the color-mapping this view exists to show.
    """
...
def process_one(image_path: str, lidar_path: str, transform_path: str,
    ...
    T, c, delta = load_transform(transform_path)
    #print(f'  T_cam_lidar:\n{np.round(T, 4)}')
    ...
    out2  = image_on_lidar(img, pts3d, T, c=c, delta=delta)
