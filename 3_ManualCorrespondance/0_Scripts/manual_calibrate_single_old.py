...
WINDOW_CAM  = 'Camera Image  [left-click=add  right-click=remove  ENTER=solve  R=reset  Q=quit]'
...
# Format: [tx, ty, tz, qx, qy, qz, qw]  (T_cam_lidar, native LiDAR frame, no X flip)
# Set to None to skip warm-start and rely on DLT only.
PRIOR_POSE = [0.025329, 0.068218, 0.168532, 0.7060836 , 0.00889139, 0.00305466, 0.70806607] # Original Images
#PRIOR_POSE = [ 0.38861704, -0.03483649,  0.56716535,  0.69748256, -0.02537514,  0.01530879, 0.7159887 ] # New Images
...
def draw_camera(img_bgr: np.ndarray, pts2d: list, result=None, pts3d_prior=None, prior_rvec=None, prior_tvec=None) -> np.ndarray:
    canvas = img_bgr.copy()
...
class CalibTool:
    ...
    def _refresh(self):
        # Determine best available rvec/tvec for diamond overlay:
        # use solver result if available, else fall back to prior
    ...
    def run(self):
        ...
        if self.result is not None:
            ...
            T[:3, 3] = self.result['t']
            print(f"T_cam_lidar:\n{np.round(T,6)}")
            print(f"Human readable rotation (x,y,z): {Rotation.from_matrix(self.result['R']).as_rotvec(degrees=True)}")
            T = np.ones([7])
            T[:3] = self.result['t']
            T[3:] = Rotation.from_matrix(self.result['R']).as_quat()
            print(T)
            print("Translation"+str(type(T[:3])))
            print(T[:3])
            print("qaternion:"+str(type(T[3:])))
            print(T[3:])

            calib_res = {
                "translation": T[:3].tolist(),
                "quaternion": T[3:].tolist(),
                "fx": FX, 
                "fy": FY, 
                "cx": CX, 
                "cy": CY,
            ...
            sname = "calibration_" + str(round(time())) + ".json"
            with open(sname, "w") as f:
                json.dump(calib_res,f,indent=2)
            print(f"\nResult saved to ./{sname}")
