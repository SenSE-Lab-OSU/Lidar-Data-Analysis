...
then solve for the 6-DoF rigid transformation
──────────────────────────────────────────────────────────────────────────────
...
# ── Camera intrinsics ──────────────────────────────────────────────────────────
scale = 2  # Default intrinsics are 1920x1080, so use 2 for 3840x2160
FX   = 1248.8 * scale
FY   = 1244.6 * scale
CX   = 945.08 * scale
CY   = 527.51 * scale
...
WINDOW_LID = (
    ...
    '[/]/←→=pair  ↑↓=rotate  scroll=zoom  mid-drag=pan]'
)
...
def solve(pts2d: np.ndarray, pts3d: np.ndarray) -> dict:
    ...
    margin = np.array([0.5, 0.5, 0.5, 1.0, 1.0, 1.0])
